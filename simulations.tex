...
\begin{exercise}
  ...
  (Hint, for  the empirical distribution function  you can use the code of Exercise~\ref{ex:1}. Before looking it up, try to recall how the cdf is computed.)
...
\section{Simulation of th $G/G/c$ in continuous time}
\label{sec:ggc-continuous-time}

In this tutorial we will write a simulator for the $G/G/c$ queue. For this we use two concepts that are essential to simulate any stochastic system of reasonable complexity: an \emph{event stack} (or event schedule) to keep track of the sequence in which  events occur,  and \emph{classes} to organize data and behavior into single logical units.  We will work in steps  towards our goal; along the way you will learn a number of fundamental and highly interesting concepts.
...
Classes are extremely useful programming concepts, as it enables you to organize state (that is, attributes) and behavior (that is, functions that apply to the attributes) into logical components. Moreover, classes can offer functionality to a programmer without the programmer needing to understand how this functionality is built. As an example, a user of the class \texttt{Job} can use the function \texttt{sojourn time} without being needing to know how to compute it. 

Once we have a class, making an object is simple.\footnote{The wording is important here. Objects are instances of classes. As an example: Albert Einstein was a human being. Here, `Einstein' is the object, and `human being' is a class.}
...
\begin{exercise}
  Run a simulation for 100 jobs and compute the average queue length. Then extend to $1000 =1e3$, and $1e5$. Compare it to the theoretical expected queue length of the $M/M/1$ at arrival moments. 
  \begin{solution}
...
\subsection{G/G/c queue, to be done}
...
Tests
\begin{itemize}
\item Compare with standard formulas for gg1
 \item $D/D/3$ in overload is evident, constructable by hand. 
\item $M/M/3$ compare the formulas
\end{itemize}
...
class GGc:
    ...
    def put_on_queue(self, time, job):
    ...
    def run(self):
        time = 0
        self.put_new_arrival_on_stack(0)
