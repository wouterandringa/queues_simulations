...
print(Q.mean(), Q.std())
\end{pyverbatim}
Then run the same code but with the line with \pyv{# marked} replaced by with \pyv{s = np.ones_like(a) * mu} (Read the docs of \pyv{numpy} to see what \pyv{np.ones_like} does).  Explain the result.
\begin{solution}
